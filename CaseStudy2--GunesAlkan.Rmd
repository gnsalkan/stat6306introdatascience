---
title: "Case Study 2 - Scraping a Job Website: CyberCoders"
author: "Gunes Alkan"
...
---
 ...
 A recently emerged field, Data Science, still does not have solid borders, and there are a lot of different perspectives and lack of clarity on the topic. One way to understand what is typically expected from Data Science or a Data Scientist is to collect available data and analyze it. With this idea we have decided to scrape / harvest a job website, and examine what skill set companies are looking for (what is the salary range of the job postings at the time, and similar aspects can be found too). Here, we will work on the job descriptions collected from the website called "CyberCoders", and R will be used as the software package. Cybercoders is a website where you can browse jobs (by location, category or name), look qualified candidates to hire for companies, so it is created with a purpose of matching companies and people across all industries.
We will, first, define our own functions to make sure that the case study is reproducible at any time by anybody, so that a broad function will use the small functions defined, and will require only the name of job (in this case it will be: Data Scientist). Here, as mentioned before salary range, and location of job can also be found, but we will rather be interested in skill sets. Finally, after scraping all Data Science job posts on CyberCoders, we'll visualize the results to make the conclusion more understandable.
...
```
 ...
 In conlcusion, through harvesting Cybercoders website and collecting - counting the keywords of job descriptions, we saw that companies who are looking for data scientists expect them to be good at machine learning, statistics, data mining, predictive analysis and so on. There are also software packages such as R, SAS, Python and Matlab that are very likely to be a part of data science job.



```{r, echo=FALSE}
# At first, when we get the assignment, when I was working on the codes I noted these as what I have learned:

## sources used (to understand XML language a little bit):
...
# in order to look at the distribution of skills, I created a list of jobinfos as the author did in the book, and then tried to use the function he used, which is:
#  tt <- sort(table(unlist(lapply(cy.dataSciPosts, '[[', "skills"))), decreasing=TRUE)
#  then I got the following error:
#  Error in FUN(X[[i]], ...) : subscript out of bounds
#  After I checked online blogs about R, I found out that the reason was about
#  - my indices exceed the array boundary (i is greater than the number of rows, or alter is greater than the 
#  number of columns)
#  - because I try to access an array / list / matrix out of his boundary
#  (further information can be found on:  http://stackoverflow.com/questions/15031338/subscript-out-of-bounds-general-definition-and-solution )
# So, I corrected the code as:
# tt <- sort(table(unlist(cy.dataSciPosts$datePosted)), decreasing=TRUE)

# one way of extracting numbers from character strings
# strr <- c("14.XYZ", "15.ABCDE", "16.dkieowo", "120.EIDKAI") 
# strr
# class(strr)
# x <- gsub("[^0-9]", "", strr) 
# x 
